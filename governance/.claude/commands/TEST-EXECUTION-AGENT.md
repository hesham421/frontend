> Adapted from backend/governance/.claude/commands/TEST-EXECUTION-AGENT.md
> on 2026-07-21 — frontend-scoped subset. If backend's copy changes in a
> way that affects shared conventions (MCP execution order, failure
> taxonomy shape), this file needs a matching manual update — there is no
> automatic sync between the two. (Note: the source file, as of this
> adaptation, contained no backend-only/JUnit content to strip out — it
> was already scoped entirely to Playwright/E2E execution, so this copy
> is a full copy of that content, not a trimmed excerpt.)
