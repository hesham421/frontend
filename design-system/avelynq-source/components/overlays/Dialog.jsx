export function Dialog({
  ...
  return (
    <div className={'avl-dialog avl-dialog--' + size} role="dialog" aria-modal="true" aria-label={title || 'Dialog'} style={{ fontFamily: 'var(--font-sans)' }}>
      <div className="avl-dialog__scrim" onClick={closeOnScrim ? onClose : undefined} />
      <div className="avl-dialog__panel" style={{
        ...
        {(title || icon || showClose) && (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '18px 20px', borderBottom: '1px solid var(--border-subtle)' }}>
            {icon && (
        ...
        <div className="avl-dialog__body" style={{ flex: '1 1 auto', padding: '20px', fontSize: 'var(--fs-body)', color: 'var(--text-body)', lineHeight: 'var(--lh-normal)' }}>
          {children}
        </div>

        {footer && (
          <div className="avl-dialog__footer" style={{ padding: '14px 20px', borderTop: '1px solid var(--border-subtle)' }}>
            {footer}
          </div>
        )}
      </div>
