import { z } from 'zod';
import type { CreateSecUserProfileRequest, UpdateSecUserProfileRequest } from '../api/userProfilesApi';
...
// (useBranchesOptions, src/branches/hooks.ts), so an inactive/nonexistent
// branch is structurally unreachable in normal use — the cross-module
// active-branch check (XM-SEC-001) remains server-authoritative for the
// race-condition case (branch deactivated between page load and submit),
// surfaced via ERR-SEC-034 (lib/errors/secErrors.ts) on mutation failure.
