...
Summarized: dictionary itself is complete and high quality (~250 keys × 2 languages, natural phrasing, not machine-translated placeholders); RTL flips the layout, sidebar position, and text alignment correctly on live testing; logical CSS properties (`insetInlineStart`, `marginInlineStart`, `textAlign: 'start'`) are used consistently in the 4 Security page components. The concrete gaps are F9 (hardcoded English table headers), F7's RTL-drawer-direction half (dead CSS), and F15/F16/F17 below (found in a later pass focused specifically on language defaulting and bilingual data).

**Not a bug — flagged and ruled out:** the "Assigned Roles" picker (`RoleAssignmentDrawer.tsx`, opened from `Users.tsx`) shows role names in English in Arabic mode too, but this is a *different* root cause from F16/F17: `RoleDto` (`roles/rolesApi.ts`) genuinely has only one `roleName` field — no `roleNameAr` exists anywhere in the type or the real backend contract it's generated from (`role-access-control.md`). Unlike Pages/Branches/Legal Entities/etc., Roles are admin-authored records with a single name, not bilingual master data, so there is no Arabic value for the frontend to switch to — inventing one client-side would be fabricating data, not fixing a bug. If Arabic role names are actually wanted, that is a backend/data-model change (a new field + migration), out of a frontend UI/UX pass's scope per this skill's "Avoiding feature creep" section.

### F15 — App defaults to English on every first visit and every refresh (Critical)
**Location:** `src/stores/useLanguageStore.ts:756-757` (`lang: 'en', dir: 'ltr'` as the Zustand store's initial state, with no `localStorage` read/write anywhere in the file); `index.html:2` (`<html lang="en" dir="ltr">`).
**Observed:** Confirmed live — clearing `localStorage` and loading `http://localhost:4200` renders the entire app in English/LTR (breadcrumbs, sidebar, KPI cards, the "العربية" toggle button all in English), even though the brief and every other part of this codebase (translation dictionary, RTL CSS, the `t()`/`lang` convention used in ~15 files) treats Arabic as the primary language. A refresh reproduces it every time because nothing ever persists the user's choice — `setLanguage`/`toggleLanguage` updated `document.documentElement` and the in-memory store but never wrote to storage.
**Why it's a UX problem:** This is the app's very first impression, on every visit, for a system whose primary audience is Arabic-speaking — and it silently discards a user's language choice on the next reload, forcing them to re-toggle every session.
**Severity:** Critical.
**Fix applied:** `useLanguageStore.ts` now reads `localStorage['avelynq_lang']` on module init (falling back to `'ar'`, not `'en'`, when unset or invalid), applies `document.documentElement.dir`/`lang` synchronously before the store is created (no LTR flash), and `setLanguage`/`toggleLanguage` now persist every change. `index.html`'s pre-hydration markup was corrected to `lang="ar" dir="rtl"` to match, so there's no flash of LTR markup before React mounts either.
**Reusable pattern?** No — single root-cause fix in one store.

### F16 — Bilingual (`nameAr`/`nameEn`) fields hardcoded to English in dropdown options and record labels, app-wide
**Location:** ~15 call sites bypassing the codebase's own `lang === 'ar' ? x.nameAr : x.nameEn` convention (already used correctly elsewhere in the same files): `PermissionMatrixDrawer.tsx` (the page-picker row label, and the search filter only matching `nameEn`), `Permissions.tsx` (target-page dropdown + table cell), `Pages.tsx` (parent-page dropdown, edit-dialog title, deactivate-confirm message), `UserProfileDrawer.tsx` and `DataScopeDrawer.tsx` (branch dropdowns), and the legal-entity/branch picker dropdowns plus edit-dialog titles in all 6 Organization screens (`Branches`, `Departments`, `CostCenters`, `ProfitCenters`, `LocationSites`, `Regions`, `LegalEntities`).
**Observed:** Live-tested in Arabic mode: the Roles & Permissions screen's Permission Matrix drawer (screenshot supplied by the user) showed page names as "Test Page," "Master Lookups," "GL Accounts," "ToggleMe" — all English — inside an otherwise fully-Arabic modal with an Arabic column header ("الشاشة / الصفحة"). The same shape reproduced in Permission Registry's "Target Page" dropdown/column and in every Organization screen's parent-entity picker. Two select options also had raw hardcoded English strings never routed through `t()` at all: `'-- None / Global --'` (Permissions.tsx) and `'-- None (Root Menu) --'` (Pages.tsx).
**Why it's a UX problem:** Same failure mode as F9 (hardcoded headers), one layer down — the column header translates but the row/option *data* doesn't, which reads as more broken, not less, because it looks like the translation was attempted and abandoned mid-record.
**Severity:** Critical (this was the user's own reported complaint, reproduced and confirmed).
**Fix applied:** Every listed call site now uses `lang === 'ar' ? x.nameAr : x.nameEn` (matching the pattern `LegalEntities.tsx`/`Regions.tsx`/etc. already used correctly elsewhere), and the two raw strings were moved into the dictionary as `noneGlobalOption`/`noneRootMenuOption`. The Permission Matrix drawer's search filter now matches against `nameAr` as well as `nameEn`, matching how every other bilingual search filter in the app already behaves (e.g. `ProfitCenters.tsx`'s `filteredProfitCenters`).
**Reusable pattern?** The *bug* repeats identically across ~15 sites (confirmed via full-repo grep for `.nameEn`, not sampled) — same fix, applied at every confirmed site; not extracted into a shared `localize()` helper in this pass since each call site's surrounding JSX/array-map shape differs enough that a helper would save one ternary per site at the cost of an extra import — worth revisiting only if a *new* bilingual-field bug surfaces and makes the drift-risk concrete (see this file's own "Consolidate the moment you notice the same logic written twice" principle in the `ui-ux` skill).

### F17 — Hardcoded English-only module labels in filter/form dropdowns (a second, distinct source of the same symptom as F16)
**Location:** `Pages.tsx` (`MODULE_OPTIONS`, both the list-screen module filter and the Add/Edit form's "الوحدة" field — the exact field in the user's second reported screenshot), `Permissions.tsx` (`MODULE_FILTERS`), `NotificationTemplates.tsx` (`moduleOptions`) — three independently hand-written arrays like `{ value: 'SEC', label: 'SEC (Security)' }`, none routed through `t()`.
**Observed:** Live-tested in Arabic: Page Registry's Edit dialog "الوحدة" dropdown showed "SEC (Security)", "ORG (Organization)", "FILE (File Service)", etc. — the labels never had an Arabic form to select in the first place, unlike F16 where an `nameAr` value existed but was ignored. Root cause is different from F16 (a static hardcoded-string array, not a bypassed bilingual DB field) even though the visible symptom — English text in an Arabic screen — is identical, which is why it survived the F16 pass (that pass grepped for `.nameEn`, and this bug has no `nameEn` in it at all).
**Why it's a UX problem:** Same as F16 — same screen, same modal, same "translation was attempted and abandoned" impression for the user.
**Severity:** High.
**Fix applied:** Extracted one shared `MODULE_LABEL_KEYS` code→translation-key map and a `getModuleLabel(code, t)` helper into `src/data/moduleLabels.ts` (new file, matching the existing `src/data/searchContract.ts` convention) — shared *because* the same bug was confirmed independently in 3 files this time (not "the same category of mistake," the literal same list), while each screen still supplies its own subset of module codes (`Pages` has 7, `Permissions` has 5, `NotificationTemplates` has 6 — not identical sets, so a single fixed option array would have been wrong). 8 dictionary keys added (`moduleSecurity`…`moduleSystemCore`) in both languages.
**Reusable pattern?** Yes — this is the shared helper F16's write-up said would be justified once a second bilingual-label bug made the drift-risk concrete; this is that second instance.
...
**Follow-up pass (language defaulting):** F15, F16, and F17 were found and fixed in a separate, later pass triggered by direct user reports — first a screenshot of the Permission Matrix drawer showing English page names in Arabic mode (F15/F16), then a second screenshot of Page Registry's "الوحدة" dropdown after F16 was already fixed, which is what surfaced F17 as a distinct root cause the F16 grep (searching for `.nameEn`) could not have caught. See those three entries above for the full before/after.
